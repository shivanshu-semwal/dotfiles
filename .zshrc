...
# conda initialize
__conda_setup="$('/home/antihero/.anaconda3/bin/conda' 'shell.zsh' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/home/antihero/.anaconda3/etc/profile.d/conda.sh" ]; then
        . "/home/antihero/.anaconda3/etc/profile.d/conda.sh"
    else
        export PATH="/home/antihero/.anaconda3/bin:$PATH"
    fi
fi
unset __conda_setup
...
todo(){
   var=$(date +"%m-%y")
   vim "$HOME/Documents/todo/$var"
}
todoo(){
   vim "$HOME/Documents/todo/$1"
}
todl(){
   /usr/bin/ls -1 "$HOME/Documents/todo"
}
...
pro(){
    if [ ! -d /media/antihero/Data/Programming/ ]; then
        return 1
    else
        VAR=$(find /media/antihero/Data/Programming -maxdepth 2 \( ! -regex '.*/\..*' \) -type d | sed 's|/media/antihero/Data/Programming/||g' | fzf --border)
        if [ ! -z "$VAR" ];then
            cd /media/antihero/Data/Programming/"$VAR"
        fi
    fi
}
...
gspeak(){
    echo "Output file: $1.mp3"
    echo "Text: ${@:2}"
    wget -q \
        -U Mozilla \
        -O $1.mp3 \
        "http://translate.google.com/translate_tts?ie=UTF-8&total=1&idx=0&textlen=32&client=tw-ob&q=${@:2}?&tl=En-gb"
}
